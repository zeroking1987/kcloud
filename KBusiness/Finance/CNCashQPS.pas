...
var
  CNCashQFM: TCNCashQFM;

implementation

uses PublicPS, ControlPublicPS, CNCashPS, CNItemQPS, CNAccNameQPS, DeptQPS, EmpQPS, PrvQPS, CustQPS,DataPS;
{$R *.dfm}

function TCNCashQFM.AuditBill(TableId, UserId, FMName, BillNo: string; qyHed: TAdoquery; iFlag: Integer): Boolean;
var
...
procedure TCNCashQFM.btAddClick(Sender: TObject);
var sAccName: string;
begin
  try
    try
      CNCashFM := TCNCashFM.Create(nil);
      CNCashFM.sOperate:='添加';
      CNCashFM.btAddClick(Sender);
      CNCashFM.qyHed.FieldByName('AccName').AsString:=cxAccName.Text;
      CNCashFM.ShowModal;
      btFindClick(Sender);
    finally
      CNCashFM.Free;
...
procedure TCNCashQFM.btCheckClick(Sender: TObject);
var
  sBiilNo: string;
begin
...
procedure TCNCashQFM.btDelClick(Sender: TObject);
var
  sBiilNo: string;
begin
  if (qyHed.FieldByName('sFlag').AsString = '期初') or (qyHed.Active = false) then Exit;
...
procedure TCNCashQFM.btEditClick(Sender: TObject);
begin
  try
    ...
    CNCashFM := TCNCashFM.Create(nil);
    CNCashFM.sOperate:='编辑';
    CNCashFM.sPubBillNo := qyHed.FieldByName('BillNo').AsString;
    CNCashFM.btEditClick(Sender);
    CNCashFM.ShowModal;
    btFindClick(Sender);
  finally
    CNCashFM.Free;
...
procedure TCNCashQFM.btExportClick(Sender: TObject);
begin
...
procedure TCNCashQFM.btFindClick(Sender: TObject);
begin
  sWhere := ' and IsNull(a.CNKind,'''')=''现金'' ';
  sWhereAccName := '  and a.AccName in(select AccName from CNAccNameTB where AccKind=''现金账户'') ';
...
procedure TCNCashQFM.btn1Click(Sender: TObject);
begin
  inherited;
...
procedure TCNCashQFM.btn2Click(Sender: TObject);
begin
  inherited;
  cxdtS.Text := FormatDateTime('yyyy-mm-dd', StartOfTheMonth(System.SysUtils.IncMonth(cxdtS.Date, 1)));
  cxdtE.Text := FormatDateTime('yyyy-mm-dd', EndOfTheMonth(System.SysUtils.IncMonth(cxdtE.Date, 1)));
...
procedure TCNCashQFM.btnFZObjectPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  try
...
procedure TCNCashQFM.btnHandPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  try
...
procedure TCNCashQFM.btUnCheckClick(Sender: TObject);
var
  sBiilNo: string;
begin
  if (qyHed.FieldByName('sFlag').AsString = '期初') or (qyHed.Active = false) then Exit;
...
procedure TCNCashQFM.cxAccNamePropertiesButtonClick(Sender: TObject);
begin
  inherited;
  try
    try
...
procedure TCNCashQFM.cxCNItemPropertiesButtonClick(Sender: TObject);
begin
  inherited;
  try
    try
...
procedure TCNCashQFM.cxdtEPropertiesEditValueChanged(Sender: TObject);
begin
  inherited;
  if cxdtS.Text<>'' then
    btFindClick(Sender);
...
procedure TCNCashQFM.cxdtSPropertiesEditValueChanged(Sender: TObject);
begin
  inherited;
  if cxdtE.Text<>'' then
  btFindClick(Sender);
...
procedure TCNCashQFM.cxgrdtbvCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
  inherited;
...
procedure TCNCashQFM.FormCreate(Sender: TObject);
begin
  sFMId := 'CNCashQFM';
...
procedure TCNCashQFM.FormDestroy(Sender: TObject);
begin
  inherited;
  CNCashQFM := nil;
end;

procedure TCNCashQFM.FormShow(Sender: TObject);
begin
  inherited;
  if sCNStartMon='' then
  begin
...
procedure TCNCashQFM.N2Click(Sender: TObject);
begin
  inherited;
  if qyHed.FieldByName('bOtherToCN').AsBoolean then
  begin
