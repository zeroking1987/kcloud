...
procedure TCNBankFM.btAddClick(Sender: TObject);
begin
  try
    inherited;
...
procedure TCNBankFM.btCancelClick(Sender: TObject);
begin
  try
    inherited;
    qyHed.CancelBatch();
...
procedure TCNBankFM.btEditClick(Sender: TObject);
begin
  try
...
procedure TCNBankFM.btSaveClick(Sender: TObject);
begin
  try
...
procedure TCNBankFM.cxAccNamePropertiesButtonClick(Sender: TObject);
begin
  inherited;
  try
    try
...
procedure TCNBankFM.cxButton1Click(Sender: TObject);
begin
  inherited;
...
procedure TCNBankFM.cxCNItemNamePropertiesButtonClick(Sender: TObject);
begin
  inherited;
  try
    try
...
procedure TCNBankFM.cxCNItemNamePropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TCNBankFM.cxFZObjectPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  if not btSave.Enabled then Exit;
...
procedure TCNBankFM.cxHandManPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  try
...
procedure TCNBankFM.cxRgpsFlagPropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TCNBankFM.FormCreate(Sender: TObject);
begin
  sTableId := 'CNBankQ';
  ShowReport(sFMId);
  ...
  begin
    ...
    SQL.Clear;
    sql.Add('select * from CNBillTB where CNKind=''银行'' ');
    Open;
  end;
end;

procedure TCNBankFM.FormDestroy(Sender: TObject);
begin
  inherited;
  CNBankFM := nil;
end;

procedure TCNBankFM.FormShow(Sender: TObject);
begin
  //inherited;
  cxlabTitle.Caption := '出纳银行账-' + sOperate;
  if sOperate = '编辑' then
  begin
    with qyHed do
    begin
...
procedure TCNBankFM.qyHedAfterInsert(DataSet: TDataSet);
begin
  inherited;
  ...
  qyHed.FieldByName('CNKind').AsString := '银行';
  qyHed.FieldByName('AutoNo').AsString := GetDateAutoNo('CNYH');
