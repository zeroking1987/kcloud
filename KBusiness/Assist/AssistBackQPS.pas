...
procedure TAssistBackQFM.btFindClick(Sender: TObject);
begin
  sWhere := '';
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' exec PrItemMoveQ ' + Quotedstr(sWhere) + ',' + Quotedstr('''AssistBack'''));
    Open;
  end;
...
procedure TAssistBackQFM.btnAcceptNamePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
...
procedure TAssistBackQFM.btnEmpNamePropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  inherited;
  SelectPrv2(btnSendName);
end;

procedure TAssistBackQFM.btnItemKindPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
...
procedure TAssistBackQFM.btnItemNamePropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  inherited;
  SelectItem2(cxItem);
end;

procedure TAssistBackQFM.cxgrdtbvDblClick(Sender: TObject);
begin
  inherited;
  BillView('AssistBack', 'select * from ItemMoveHDTB where MKindId=''AssistBack'' and  MoveNo=' + QuotedStr(qyHed.FieldByName('MoveNo').AsString));
end;

procedure TAssistBackQFM.cxTechNamePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  try
...
procedure TAssistBackQFM.FormCreate(Sender: TObject);
begin
  sFMId := 'AssistBackQFM';
  sFMName := Self.Caption;
  sTableId := 'AssistBackQ';
  inherited;
end;

procedure TAssistBackQFM.FormDestroy(Sender: TObject);
begin
  inherited;
  AssistBackQFM := nil;
end;

procedure TAssistBackQFM.FormShow(Sender: TObject);
begin
  inherited;
  with qyOutWare do
  begin
