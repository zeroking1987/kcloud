...
uses
  ...
  cxNavigator, cxRadioGroup, cxBarEditItem, cxTrackBar;

type
  TAssistMoveFM = class(TBaseGrdFM)
    ...
    qyDetInWare: TStringField;
    qyDetInSjNo: TStringField;
    qyDetInsOrderNo: TStringField;
    qyDetInTechName: TStringField;
    qyDetInInDate: TStringField;
    qyDetInPState: TStringField;
    qyDetInTempNo: TStringField;
...
var
  AssistMoveFM: TAssistMoveFM;

implementation

uses PublicPS, ControlPublicPS, DataPS, EmpQPS, ItemQPS, WareQPS, DeptQPS, PrvQPS, ItemMoveFindPS, TechOutProDispatchRfPS,MatProDispatchRfPS;
{$R *.dfm}

procedure TAssistMoveFM.btAddClick(Sender: TObject);
begin
  try
...
procedure TAssistMoveFM.btAddLClick(Sender: TObject);
begin
  try
    qyDet.Append;
  except
...
procedure TAssistMoveFM.btCancelClick(Sender: TObject);
begin
  qyDet.CancelBatch();
  qyHed.CancelBatch();
...
procedure TAssistMoveFM.btCheckClick(Sender: TObject);
begin
  try
...
procedure TAssistMoveFM.btDelClick(Sender: TObject);
var sBiilNo: string;
begin
...
procedure TAssistMoveFM.btDelLClick(Sender: TObject);
begin
  try
    qyDet.Delete;
  except
...
procedure TAssistMoveFM.btEditClick(Sender: TObject);
var i: integer;
begin
...
procedure TAssistMoveFM.btFindClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TAssistMoveFM.btRefClick(Sender: TObject);
begin
  try
    inherited;
...
procedure TAssistMoveFM.btSaveClick(Sender: TObject);
var
  sNewNo: string;
  i: Integer;
begin
...
procedure TAssistMoveFM.btUnCheckClick(Sender: TObject);
begin
  try
    if (trim(cxEdtNo.Text) = '') or (qyHed.Active = false) then exit;
    qyHed.UpdateCursorPos;
...
procedure TAssistMoveFM.cxBt1PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  inherited;
  try
...
procedure TAssistMoveFM.cxBt2PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  inherited;
  try
    if btSave.Enabled = False then Exit;
...
procedure TAssistMoveFM.cxBtnInWarePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
...
procedure TAssistMoveFM.cxBtnOutWarePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
...
procedure TAssistMoveFM.cxEdtNoPropertiesChange(Sender: TObject);
begin
  inherited;
  with qyDet do
  begin
...
procedure TAssistMoveFM.cxgrdbclmnColumn1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  try
...
procedure TAssistMoveFM.cxgrdbclmnColumn6PropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  inherited;
  Error := false;
...
procedure TAssistMoveFM.cxgrdbclmnItemCodePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  i: Integer;
begin
...
procedure TAssistMoveFM.cxgrdbclmnItemPYCodePropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  inherited;
...
procedure TAssistMoveFM.cxgrdbclmnOrderNumPropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  inherited;
  Error := False;
...
procedure TAssistMoveFM.cxgrdbclmnOrderPricePropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  inherited;
  Error := False;
...
procedure TAssistMoveFM.cxgrdbclmnTaxPricePropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  try
    inherited;
...
procedure TAssistMoveFM.cxgrdbclmnTaxRatePropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  try
    inherited;
...
procedure TAssistMoveFM.cxgrdtbvCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
var
  ARec: TRect;
begin
  inherited;
  if (AViewInfo.GridRecord.DisplayTexts[cxgrdbclmnMoveNum.Index] <> '') and (AViewInfo.GridRecord.DisplayTexts[cxgrdWareNum.Index] <> '') then
  begin
    if (not qyHed.FieldByName('bMaxAudit').AsBoolean) and (StrToFloat(AViewInfo.GridRecord.DisplayTexts[cxgrdbclmnMoveNum.Index]) > StrToFloat(AViewInfo.GridRecord.DisplayTexts[cxgrdWareNum.Index])) then
    begin
...
procedure TAssistMoveFM.dxBarButton1Click(Sender: TObject);
var i:Integer;
begin
try
    MatProDispatchRfFM := TMatProDispatchRfFM.Create(nil);
    MatProDispatchRfFM.sKind := '外协发料';
...
procedure TAssistMoveFM.dxBarButton2Click(Sender: TObject);
var i:Integer;
begin
try
      TechOutProDispatchRfFM := TTechOutProDispatchRfFM.Create(nil);
      TechOutProDispatchRfFM.sKind := '外协工序出库';
    //TechOutProDispatchRfFM.sObjectId := qyHed.FieldByName('AcceptId').asString;

      TechOutProDispatchRfFM.btFindClick(self);
      TechOutProDispatchRfFM.ShowModal;
      if TechOutProDispatchRfFM.fFlag = 1 then
      begin
        TechOutProDispatchRfFM.qyFind.Filter := ' bSelect=1';
        TechOutProDispatchRfFM.qyFind.Filtered := true;
        TechOutProDispatchRfFM.qyFind.First;
        for i := 0 to TechOutProDispatchRfFM.qyFind.RecordCount - 1 do
        begin
          if i = 0 then
            qyDet.Edit
          else
            qyDet.Append;
          qyDet.FieldByName('ItemId').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('ItemId').AsString;
          qyDet.FieldByName('ItemCode').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('ItemCode').AsString;
          qyDet.FieldByName('CompactNo').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('CompactNo').AsString;
          qyDet.FieldByName('InCompactNo').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('CompactNo').AsString;
          qyDet.FieldByName('ProType').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('ProType').AsString;
          qyDet.FieldByName('InProType').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('ProType').AsString;
          qyDet.FieldByName('Color').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('Color').AsString;
          qyDet.FieldByName('inColor').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('Color').AsString;
          qyDet.FieldByName('Ordinal').AsInteger := TechOutProDispatchRfFM.qyFind.FieldByName('Ordinal').AsInteger;
          qyDet.FieldByName('TopTech').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('TopTech').AsString;
          qyDet.FieldByName('bFirst').AsBoolean := TechOutProDispatchRfFM.qyFind.FieldByName('bFirst').AsBoolean;
          qyDet.FieldByName('bLast').AsBoolean := TechOutProDispatchRfFM.qyFind.FieldByName('bLast').AsBoolean;

          qyDet.FieldByName('MoveNum').AsFloat := TechOutProDispatchRfFM.qyFind.FieldByName('NoRfNum').AsFloat;
          qyDet.FieldByName('RfNum').AsFloat := TechOutProDispatchRfFM.qyFind.FieldByName('NoRfNum').AsFloat;

          qyDet.FieldByName('SaleOrderNo').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('SaleOrderNo').AsString;
          qyDet.FieldByName('SaleOrderId').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('SaleOrderId').AsString;
          qyDet.FieldByName('ProOrderNo').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('ProOrderNo').AsString;
          qyDet.FieldByName('ProOrderId').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('ProOrderId').AsString;
          qyDet.FieldByName('DispatchNo').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('DispatchNo').AsString;
          qyDet.FieldByName('DispatchId').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('AutoNO').AsString;
          qyDet.FieldByName('TechName').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('TechName').AsString;
          qyDet.FieldByName('InTechName').AsString := TechOutProDispatchRfFM.qyFind.FieldByName('TechName').AsString;

          UpdateData(cxgrdtbv, 'MoveNum');
          qyDet.post;
          TechOutProDispatchRfFM.qyFind.Next;
        end;
      end;
    finally
      TechOutProDispatchRfFM.free;
      TechOutProDispatchRfFM := nil;
    end;
end;

procedure TAssistMoveFM.dxbrbtn1Click(Sender: TObject);
begin
  inherited;
...
procedure TAssistMoveFM.dxbrbtn2Click(Sender: TObject);
begin
  inherited;
...
procedure TAssistMoveFM.FormCreate(Sender: TObject);
begin
  sFMId := 'AssistMoveFM';
...
procedure TAssistMoveFM.FormDestroy(Sender: TObject);
begin
  inherited;
  AssistMoveFM := nil;
end;

procedure TAssistMoveFM.FormShow(Sender: TObject);
var i: Integer;
begin
  inherited;
  TextReadOnly(Self, True);
...
procedure TAssistMoveFM.qyDetAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TAssistMoveFM.qyDetCalcFields(DataSet: TDataSet);
begin
  inherited;
...
procedure TAssistMoveFM.qyHedCalcFields(DataSet: TDataSet);
begin
  inherited;
...
procedure TAssistMoveFM.qyHedPrintNumChange(Sender: TField);
begin
  inherited;
  ShowFMInfo(sTableId, qyHed);
end;

procedure TAssistMoveFM.strngfldDetItemIdChange(Sender: TField);
begin
  inherited;
