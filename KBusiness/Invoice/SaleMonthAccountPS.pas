...
type
  TSaleMonthAccountFM = class(TBase1FM)
    ...
    pnl1: TPanel;
    qyHedbSaleAccount: TBooleanField;
...
var
  SaleMonthAccountFM: TSaleMonthAccountFM;

implementation
uses DataPS, PublicPS, WareInOutQPS;
{$R *.dfm}

procedure TSaleMonthAccountFM.btCheckClick(Sender: TObject);
var
  dtS, dtE: string;
begin
  try
...
procedure TSaleMonthAccountFM.btExitClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TSaleMonthAccountFM.btFindClick(Sender: TObject);
begin
  inherited;
  qyHed.Open;
end;

procedure TSaleMonthAccountFM.btUnCheckClick(Sender: TObject);
begin
  try
    inherited;
    try
...
procedure TSaleMonthAccountFM.FormCreate(Sender: TObject);
begin
  sFMId := 'SaleMonthAccountFM';
  sFMName := Self.Caption;
  sTableId := 'SaleMonthAccount';
  sTableName := cxlabTitle.Caption;
  inherited;
  btFindClick(Sender);
end;

procedure TSaleMonthAccountFM.FormDestroy(Sender: TObject);
begin
  inherited;
  SaleMonthAccountFM := nil;
end;
