...
procedure TStockInQFM.btFindClick(Sender: TObject);
begin
  sWhere := '';
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' exec PrStockInQ ' + Quotedstr(sWhere));
    Open;
  end;
end;

procedure TStockInQFM.btnObjectPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
...
procedure TStockInQFM.cxbtLastMClick(Sender: TObject);
begin
  inherited;
...
procedure TStockInQFM.cxbtNextMClick(Sender: TObject);
begin
  inherited;
  cxdtS.Text := FormatDateTime('yyyy-mm-dd', StartOfTheMonth(System.SysUtils.IncMonth(cxdtS.Date, 1)));
  cxdtE.Text := FormatDateTime('yyyy-mm-dd', EndOfTheMonth(System.SysUtils.IncMonth(cxdtE.Date, 1)));
...
procedure TStockInQFM.cxbtNowMClick(Sender: TObject);
begin
  inherited;
...
procedure TStockInQFM.cxgrdtbvDblClick(Sender: TObject);
begin
  inherited;
  BillView('StockIn', 'select * from WareInOutHDTB where BillNo=' + QuotedStr(qyHed.FieldByName('BillNo').AsString));
end;

procedure TStockInQFM.cxItemPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  SelectItem2(cxItem);
...
procedure TStockInQFM.FormCreate(Sender: TObject);
begin
  sFMId := 'StockInQFM';
...
procedure TStockInQFM.FormDestroy(Sender: TObject);
begin
  inherited;
  StockInQFM := nil;
end;

procedure TStockInQFM.FormShow(Sender: TObject);
var
  i: Integer;
begin
  inherited;
