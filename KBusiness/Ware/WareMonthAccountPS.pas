...
type
  TWareMonthAccountFM = class(TBase1FM)
...
var
  WareMonthAccountFM: TWareMonthAccountFM;

implementation
uses DataPS, PublicPS, WareInOutQPS;
{$R *.dfm}

procedure TWareMonthAccountFM.btCheckClick(Sender: TObject);
var
  dtS, dtE: string;
begin
  try
    ...
    try
      ...
      begin
        ...
        Parameters.Clear;
        Procedurename := 'prMonthAccount;1';
...
procedure TWareMonthAccountFM.btExitClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TWareMonthAccountFM.btFindClick(Sender: TObject);
begin
  inherited;
  qyHed.Open;
end;

procedure TWareMonthAccountFM.btUnCheckClick(Sender: TObject);
begin
  try
    inherited;
    try
...
procedure TWareMonthAccountFM.FormCreate(Sender: TObject);
begin
  sFMId := 'WareMonthAccountFM';
  sFMName := Self.Caption;
  sTableId := 'WareMonthAccount';
  sTableName := cxlabTitle.Caption;
  inherited;
  btFindClick(Sender);
end;

procedure TWareMonthAccountFM.FormDestroy(Sender: TObject);
begin
  inherited;
  WareMonthAccountFM := nil;
end;
